...
const AddEditBlogCategory = ({ token, data, isEdit, id }: any) => {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (isValid) {
        ...
        if (status) {
          toast.success("Successfully Added Blog Category");
          setFormError(defaultError);
          clearCachesByServerAction("/admin/form/blog-category");
          router.push("/admin/form/blog-category");
        } else {
          toast.error("Error While Adding Blog Category");
          setLoading(false);
    ...
    try {
      ...
      if (isValid) {
        const response = await JsonPatch(
          ...
          const { status }: any = response;
          if (status) {
            toast.success("Successfully Updated Blog Category");
            setFormError(defaultError);
            clearCachesByServerAction("/admin/form/blog-category");
            router.push("/admin/form/blog-category");
          } else {
            toast.error("Error While Updating Blog Category");
            setLoading(false);
          }
        } else {
          toast.error("Validation Error");

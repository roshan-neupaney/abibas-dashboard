...
const NewsCategory = ({ _data, token }: any) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (status) {
        toast.success("News category successfully deleted");
        router.refresh();
        toggleModal(defaultStateModal);
      } else {
        toast.error("Error while deleting News category");
      }
    } catch (e) {
      toast.error("Error while deleting News category");
    }
        ...
        >
          {data?.length > 0 ? (
            <>
              ...
              <CustomTableBody
                entireRoute="/admin/news/news-category/edit/"
                {...{ table, toggleModal }}
              />
            </>
          ) : (
